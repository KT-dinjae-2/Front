...
const mockMonthlyData = {
  '왕십리2동': [
    { 
      ...
      name: '송하정', 
      transactions: [
        { year: 2025, month: 1, type: 'donation', count: 2, amount: 20000 }, { year: 2025, month: 1, type: 'share', count: 1, amount: 10000 },
        { year: 2025, month: 2, type: 'donation', count: 1, amount: 10000 }, { year: 2025, month: 2, type: 'share', count: 1, amount: 10000 },
        { year: 2024, month: 12, type: 'donation', count: 3, amount: 35000 }, // 2024년 데이터 예시
      ]
    },
    { 
      id: '2', 
      name: '돌삼겹나들목',
      transactions: [
        { year: 2025, month: 1, type: 'donation', count: 1, amount: 25000 }, { year: 2025, month: 3, type: 'donation', count: 1, amount: 15000 },
      ]
    },
    { 
      id: '3', 
      name: '파리바게뜨 왕십리무학점',
      transactions: [
        { year: 2025, month: 1, type: 'donation', count: 2, amount: 20000 }, { year: 2025, month: 1, type: 'share', count: 3, amount: 30000 },
        { year: 2025, month: 2, type: 'donation', count: 2, amount: 30000 }, { year: 2025, month: 2, type: 'share', count: 2, amount: 20000 },
      ]
    },
  ],
  '마장동': [
  ...
  <View style={styles.card}>
    ...
    <View style={styles.statsRow}>
      <View style={styles.statItem}>
        <Text style={styles.statLabel}>기부건수</Text>
        <Text style={styles.statValue}>{item.donationCount}건</Text>
      </View>
      <View style={styles.statItem}>
        <Text style={styles.statLabel}>기부 금액</Text>
        <Text style={styles.statValue}>{formatNumber(item.donationAmount)}원</Text>
      </View>
    </View>
    <View style={styles.statsRow}>
      <View style={styles.statItem}>
        <Text style={styles.statLabel}>나눔건수</Text>
        <Text style={styles.statValue}>{item.shareCount}건</Text>
      </View>
      <View style={styles.statItem}>
        <Text style={styles.statLabel}>나눔 금액</Text>
        <Text style={styles.statValue}>{formatNumber(item.shareAmount)}원</Text>
      </View>
    </View>
...
const DongDashboardScreen = ({ route, navigation }) => {
  ...
  const [displayData, setDisplayData] = useState({ stores: [], totalDonations: 0 });
  ...
  useEffect(() => {
    ...
    const filteredStores = originalStores.map(store => {
      const transactions = store.transactions.filter(t => {
        const yearMatch = selectedYear === '전체' || t.year === selectedYear;
        const monthMatch = selectedMonth === '전체' || t.month === parseInt(selectedMonth);
    ...
    const totalDonations = filteredStores.reduce((sum, store) => sum + store.donationCount, 0);

    setDisplayData({ stores: filteredStores, totalDonations });
  ...
  const togglePicker = (picker) => {
    ...
    if (picker === 'year') {
      ...
      setIsMonthPickerVisible(false); // 다른 피커는 닫기
    } else {
      setIsMonthPickerVisible(!isMonthPickerVisible);
      setIsYearPickerVisible(false); // 다른 피커는 닫기
    }
  };
  ...
  const renderListHeader = () => (
    <>
      <View style={styles.header}>
        ...
        <Text style={styles.subtitle}>총 기부건수 : {displayData.totalDonations}</Text>
      </View>
        ...
        <View style={styles.filterRow}>
            ...
            {isYearPickerVisible && (
              <View style={styles.pickerGrid}>
                {availableYears.map(year => (
                  <TouchableOpacity key={year} style={styles.pickerItem} onPress={() => handleSelectYear(year)}>
          ...
          <View style={styles.pickerWrapper}>
            ...
            {isMonthPickerVisible && (
              <View style={styles.pickerGrid}>
                {months.map(month => (
                  <TouchableOpacity key={month} style={styles.pickerItem} onPress={() => handleSelectMonth(month)}>
                    <Text style={[styles.pickerItemText, selectedMonth === month && styles.selectedPickerItemText]}>{month}</Text>
                  </TouchableOpacity>
                ))}
              </View>
            )}
      ...
      <FlatList
        ...
        ListEmptyComponent={
          <View style={styles.emptyContainer}>
            <Text style={styles.emptyText}>해당 기간의 데이터가 없습니다.</Text>
          </View>
        }
      />
    </SafeAreaView>
